...
def convert_labels_categorical(labelset, labels, return_label_to_int_map=False):
    ...
    ----------
    labelset : str
        that represents classes of labels for a given bird
        for example, 'iabcdefg'
    labels : ndarray
        sequence of labels from a song annotation file
    return_label_to_int_map : bool
        if True, returns dict that maps labelset to
        consecutive integers 0 to n-1, where n is
        number of label classes.
        Default is False.

    Returns
    -------
    labels_categorical : ndarray
        one-hot vector encoding of labels
    labels_as_int : ndarray
        labels converted to int
    label_to_int_map : dict
        that maps labels (characters) to
        consecutive integers 0 to n-1, where n is
        number of label classes.
        Only returned if return_label_to_int_map is True.
    """
    ...
    label_to_int_map = dict(zip(labelset, classes_zero_to_n))
    labels_zero_to_n = np.asarray([label_to_int_map[label] for label in labels])
    # so we can then convert to array of binary / one-hot vectors for training
    if return_label_to_int_map:
...
def convert_int_to_labels(labels_int, label_to_int_map):
    """convert back from integer labels to character labels.

    Inverts the dict `label_to_int_map` and uses that to convert back.

    Used by hvc.predict for applying predictions from neural network
    (that outputs  predictions in the form of one-hot vectors or integers)
    and used by hvc.select for comparing predictions to test data
    that is in the form of characters, not integer labels.

    Parameters
    ----------
    labels_int : ndarray
        output from keras.model.predict_classes
    label_to_int_map : dict
        where keys are character labels and values are ints.
        Maps labels (characters) to consecutive integers 0 to n-1,
        where n is number of label classes.
    ...
    return np.asarray([int_to_label_map[an_int] for an_int in labels_int])
