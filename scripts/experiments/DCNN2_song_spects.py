...
# below is ugly, yes, but I need the dictionary in the order above and intact
# for within the model function
for layer,filter_size_or_stride in OrderedDict(reversed(list(LAYERS_DICT.items()))).items(): 
    if 'conv' in layer:
        addition += filter_size_or_stride-1
    elif 'pool' in layer:
        addition *= filter_size_or_stride
        scaling_factor *= filter_size_or_stride

input_layer_length = int(scaling_factor * np.ceil((num_timebins_for_max_spect - addition) / scaling_factor) + addition)
length_for_all_spects = input_layer_length + scaling_factor - 1
...
#correct label is '7'.
train_spects_size = (len(train_spects_padded),) + input_shape
train_spects = np.zeros(train_spects_size)

label_vec_indices = np.asarray(range(0,label_vector_length,8))
train_labels_size = (len(train_spects_padded),) + (scaling_factor,) + (len(label_vec_indices),) + (train_labels_padded[0].shape[1],)
train_labels = np.zeros(train_labels_size)
                     
shift_label_vector_length = len(label_vec_indices)
for ind,spect,label_vec in zip(range(len(train_spects_padded)),
                           train_spects_padded,
                           train_labels_padded):
    
    for shift in range(0,scaling_factor):  # scaling factor determined above from LAYERS_DICT
        if shift == 0:
            train_spects[ind,shift,0,:,shift:] = spect
        elif shift > 0:
            train_spects[ind,shift,0,:,shift:] = spect[:,:-shift]
        
        curr_song_label_vec = label_vec[label_vec_indices+shift,:]
        train_labels[ind,shift] = curr_song_label_vec
...
#train_labels = train_labels.reshape(n,y,x)  #to match output of DCNN model

#set up to save after each epoch
filepath = "dcnn2-weights-{epoch:02d}-{acc:.2f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='acc', verbose = 1, save_best_only=True)
csv_logger = CSVLogger('C://DATA//koumura birds/Bird0//DCNN2_Bird0_run1.log',
                       separator=',',
                       append=True)
