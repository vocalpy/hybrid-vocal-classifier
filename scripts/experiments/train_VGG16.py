...
WAVE_DIR = '/home/ildefonso/data/Bird0/Wave'
ANNOTATION_FILE = '/home/ildefonso/data/Bird0/Annotation.xml'

song_list = hvc.utils.utils.parse_xml(ANNOTATION_FILE,
...
if np.min(syl_counts) < 10 ** np.floor(np.log10(np.mean(syl_counts))):
    raise ValueError("One class of syllable occurs oimprders of magnitude less"
...
for song in song_list[:20]:
    ...
    if sampfreq != SAMP_FREQ:
        raise ValueError(
            ...
            'frequency of {}'.format(seq.wavFile,
                                     sampfreq,
                                     SAMP_FREQ))
    song_wav = wav[song.position:(song.position+song.length)]
    spect_obj = hvc.utils.utils.make_spect(song_wav,sampfreq,
                                           size=WINDOW_SIZE,
                                           step=WINDOW_STEP,
