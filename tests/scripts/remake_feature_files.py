...
def main():
    ...
    for feature_to_create in feature_files_to_create:
        extract_config = TEST_CONFIGS.joinpath(f'test_extract_{feature_to_create}.config.yml')
        print('running {} to create feature file'.format(extract_config))
        replace_dict = {'output_dir': ('replace with tmp_output_dir', str(FEATURE_FILES_DST))}
        ...
        shutil.move(src=features_created,
                    dst=FEATURE_FILES_DST.joinpath(movename))
