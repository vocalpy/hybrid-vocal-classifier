...
def main():
    for model_name, (select_config,
        ...
        if len(select_output_dir) != 1:
            raise ValueError('incorrect number of outputs when looking for select '
        ...
        shutil.move(src=str(meta_file),
                    dst=str(meta_file_dst))
