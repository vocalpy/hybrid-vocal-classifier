...
def some_syllables(test_data_dir):
    ...
    songfiles_list = glob(songfiles_dir)
    first_song = songfiles_list[0]
    annot_dict = hvc.utils.annotation.notmat_to_annot_dict(notmat=first_song + '.not.mat')
    ...
    some_syllables = hvc.audiofileIO.make_syls(raw_audio=raw_song,
                                               samp_freq=samp_freq,
...
class ScalarFeaturesTestClass:
    def _assert_scalar_features(self,
                                a_list_of_syllables,
                                scalar_type_func):
